...
static char *cwd_name(void) {
    char cwd[4096];
    if (!getcwd(cwd, sizeof(cwd))) return strdup("a.out");
...
i32 section_load(const char *path, const char *section_name, Section *out) {
    ...
    if (ini && iniparser_getnsec(ini) > 0) {
        ...
        if (sec0) project = strdup(sec0);
    }
    if (!project) project = cwd_name();
    ...
    if (have_section) {
        ...
        char compiler_key[300], flags_key[300], include_key[300],
             source_key[300], output_key[300];
        ...
        out->has_build = iniparser_find_entry(ini, compiler_key) ||
                          iniparser_find_entry(ini, flags_key) ||
                          iniparser_find_entry(ini, include_key) ||
                          iniparser_find_entry(ini, source_key) ||
                          iniparser_find_entry(ini, output_key);
...
static char *assemble_build_command(const Section *s) {
    isize cap = (isize)strlen(s->compiler) + (isize)strlen(s->source) +
                (isize)strlen(s->output) + 32;
    if (s->flags)   cap += (isize)strlen(s->flags) + 1;
    if (s->include) cap += (isize)strlen(s->include) + 4;
    ...
    bool is_dir = slen == 0 || s->source[slen - 1] != 'c'; // crude .c check
    if (is_dir) {
        if (slen > 0 && s->source[slen - 1] != '/') strcat(cmd, "/");
        strcat(cmd, "*.c");
    }
...
char *section_build_command(const Section *s) {
    ...
    if (s->command && build_cmd) {
        ...
        if (!chained) { free(build_cmd); return NULL; }
        snprintf(chained, len, "%s && %s", s->command, build_cmd);
        free(build_cmd);
        return chained;
    }

    if (s->command) return strdup(s->command);
